//! Canonical host encoders for `QRS2` and its quad-native `QRP2` payloads.

use quake_formats::{
    RenderQuad, RenderQuadCommand, RenderQuadObject, RenderQuadPayload, RenderQuadRun,
    RenderSectionDirectory, RENDER_QUAD_CELL_BYTES, RENDER_QUAD_COMMAND_BYTES,
    ...
    RENDER_QUAD_PROJECTED_POSITION_BYTES, RENDER_QUAD_RECORD_BYTES, RENDER_QUAD_RUN_BYTES,
...
pub fn encode_render_quad_payload(
    ...
    output[10..12].copy_from_slice(&quad_count.to_le_bytes());
    ...
    output[44..48].copy_from_slice(&file_bytes_u32.to_le_bytes());
    output[48..52].copy_from_slice(&packet_pool_bytes.to_le_bytes());
    output[52..56].copy_from_slice(&projection_bytes.to_le_bytes());

    for (index, object) in input.objects.iter().enumerate() {
        let start = objects_offset + index * RENDER_QUAD_OBJECT_BYTES;
        output[start..start + 2].copy_from_slice(&object.first_quad.to_le_bytes());
        output[start + 2..start + 4].copy_from_slice(&object.quad_count.to_le_bytes());
        output[start + 4..start + 6].copy_from_slice(&object.first_run.to_le_bytes());
        output[start + 6..start + 8].copy_from_slice(&object.run_count.to_le_bytes());
        for axis in 0..3 {
            output[start + 8 + axis * 2..start + 10 + axis * 2]
                .copy_from_slice(&object.mins[axis].to_le_bytes());
            output[start + 14 + axis * 2..start + 16 + axis * 2]
                .copy_from_slice(&object.maxs[axis].to_le_bytes());
        }
        output[start + 20..start + 22].copy_from_slice(&object.flags.to_le_bytes());
        output[start + 22..start + 24].copy_from_slice(&object.plane.to_le_bytes());
    }
    for (index, quad) in input.quads.iter().enumerate() {
        let start = quads_offset + index * RENDER_QUAD_RECORD_BYTES;
        for (position, value) in quad.positions.iter().enumerate() {
            output[start + position * 2..start + position * 2 + 2]
                .copy_from_slice(&value.to_le_bytes());
        }
        for (word, value) in quad.invariant_words.iter().enumerate() {
            output[start + 8 + word * 4..start + 12 + word * 4]
                .copy_from_slice(&value.to_le_bytes());
        }
    }
    ...
    for (section_index, section) in sections.iter().enumerate() {
        let parsed = RenderQuadPayload::parse(&section.payload.bytes).map_err(|error| {
            CookError::new(format!(
                "render section {section_index} has invalid QRP2: {error:?}"
            ))
        })?;
        ...
        let active = u32::try_from(section.payload.bytes.len())
            .ok()
            .and_then(|bytes| bytes.checked_add(dual_packet_pool_bytes))
            .and_then(|bytes| bytes.checked_add(section.payload.projection_bytes))
...
mod tests {
    ...
    fn quad_payload_roundtrips_exact_packet_and_projection_accounting() {
        ...
        assert_eq!(parsed.object(0).unwrap().plane, 5);
    ...
    fn directory_proves_active_plus_neighbor_preload_fits() {
        ...
        let active = sections[0].payload.bytes.len()
            + sections[0].payload.packet_pool_bytes as usize * 2
            + sections[0].payload.projection_bytes as usize
            + sections[0].fallback_bytes as usize;
    ...
    fn directory_rejects_a_neighbor_preload_over_budget() {
        ...
        assert!(encode_render_sections(&[0, 1], &sections, 1, 1).is_err());
    }
}
