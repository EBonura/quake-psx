...
enum Action {
    ...
    E1m1GpuPolygonSelectionKernelBench,
...
fn real_main() -> Result<()> {
    ...
    match cli.action {
        ...
        Action::E1m1GpuPolygonSelectionKernelBench => {
            let pak = resolve_pak(&root, cli.quake_dir.as_deref())?;
            cook_assets(&root, &pak, false)?;
            let build = root.join("build-psoxide-e1m1-gpu-polygon-selection-kernel-bench");
            fs::create_dir_all(&build)?;
            request_guest_link_map(build.join("quake-psx.map"))?;
            build_disc(
                &root,
                &build,
                Some(
                    "e1m1-chain-regression,perf-fixed-ticks,renderer-selection-cache,renderer-block-frustum,renderer-gpu-polygon-clip,renderer-cell-policy,renderer-mips-selection-kernel",
                ),
                false,
            )?;
            let frontend = resolve_frontend(&root, cli.psoxide.as_deref())?;
            run_e1m1_chain_regression(
                ...
                &frontend,
                &build,
                "e1m1-gpu-polygon-selection-kernel-bench",
            )?;
        }
    ...
    println!(
        "quake-psx-build\n\n\
         ...
         Actions:\n  \
           ...
           e1m1-gpu-polygon-selection-kernel-bench  A/B fixed MIPS-I/GTE face-selection microkernel\n  \
...
fn audit_ignored_top(top: &str) -> bool {
    matches!(
        ...
        ".git"
            ...
            | "build-psoxide-e1m1-gpu-polygon-selection-kernel-bench"
